...
function traer_carrito($email_usuario, $conn){
    ...
    $query = "SELECT carrito.*, usuarios.email
                FROM carrito 
...
function traer_carrito_productos($id_carrito, $conn){
    // Me fijo si existe el carrito
    $query = "SELECT carrito_productos.*, producto.nombre as titulo, producto.precio as precio, producto.descuento as descuento, (SELECT imagen FROM producto_imagenes WHERE id_producto = carrito_productos.id_producto LIMIT 1) as src, producto_atributo.nombre as atributo
        ...
        <ul class="carrito_productos" id="productos">
            <?php 
                ...
                while($carrito_producto = $carrito_productos->fetch_assoc()){
                    $descuento = 0;
                    if(isset($carrito_producto["descuento"]) && $carrito_producto["descuento"]){
                        $descuento = $carrito_producto["descuento"];
                    }
                    $precioFinal = $carrito_producto['precio'] - $carrito_producto['precio']*($descuento/100);;
                    echo('
                        <li id="carrito_producto_'.$carrito_producto["id"].'" precio="'.$precioFinal * $carrito_producto["cantidad"].'">
                            ...
                            <div class="info">
                                ...
                                <div class="cantidad">
                                ');
                    if($precioFinal == $carrito_producto["precio"]){
                        // no hay descuento
                        echo('<p>Precio unit.: $'. number_format($carrito_producto["precio"], 2) . '</p>');
                    }
                    else{
                        echo('<p>Precio unit.: <span style="text-decoration: line-through;">$'. number_format($carrito_producto["precio"], 2) . '</span><span>$'.number_format($precioFinal, 2).'</span></p>');
                    }
                    echo('
                                    <p>Cantidad: '.$carrito_producto["cantidad"].'</p>
                                </div>
                                <div class="precio_item">
                                    $'.number_format($precioFinal * $carrito_producto["cantidad"], 2).'
                                    <button class="eliminar" onclick="eliminarProductoCarrito('.$carrito_producto["id"].')">X</button>
                                </div>
                            </div>
                        </li>
                    ');
                    $subtotal += $precioFinal * $carrito_producto["cantidad"];
                }
                echo '</ul>';
                echo '
                    <div class="subtotal">
                        ...
                        <div class="info">
                            ...
                            <div class="precios">
                                <span>Subtotal:</span>
                                <p id="precioTotal" precio="'.$subtotal.'">$'. number_format($subtotal, 2) .'</p>
                            </div>
                        </div>
